...
If you intend to force <code> digitalWrite(21, LOW) </code>  to shutdown power to the GPS/LoRa/USB and achieve low power during ESP32 operation you should see the later section [Part 3: Push button to enter (all ON) firmware programming mode](#part-3). <b> If you don't implement the push-botton switch as I suggested in [Part 3](#part-3) then, if the worst happens</b> and you force <code> digitalWrite(21, LOW) </code> and the code you uploaded keeps <code>GPIO21=LOW </code> you will not be able to program the TTGO and it is effectively "bricked".  However the simple solution is to remove the ME6211/EN (Pin 5) link to GPIO21 and tie this pin high (3.3V or 5V) via a 10 k&Omega; resistor.  This will re-enable the USB controller and you can upload a modified version of the firmware.  To avoid this, I recommend implementing the [Push button to enter firmware programming mode](#part-3).
...
## Part 3 
<b>Push button to enter (all ON) firmware programming mode</b><br>
